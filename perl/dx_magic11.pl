...
# original order is not needed but makes debugging from CAD so much easier
    ...
    my $attrib_orphan_count = 0; # AcDbAttribute fields which came after an orphan value, prog style
...
# print " ATTRIBUTE: blockname for $handle is $blockname, state is $state, line is $line, attribute value orphans: $attrib_orphan_count\n";
            }
  ...
  #  }
            ...
            # If value came before the AcDbAttribute then this line should match both conditions:
            # ATTRIBUTE comes before value for CAD and value before ATTRIBUTE for prog - then the key tag name
            elsif ( ($state eq 'ATTRIBUTE' or $state eq 'TAGVALUE') && $line =~ /^[ ]{2}2\r?\n/ ) {
                $state = 'TAG';
            }
            elsif ( $state eq 'TAG' ) {
...
"\n Finished parsing $xfile \n (DXX file or DXF of unknown version)\n";
    ...
    print "\n Orphan value before AcDbAttribute count: $attrib_orphan_count (zero, for CAD dxx which used SEQEND and AcDbSequenceEnd flags)\n SEQEND count: $seqend_count, AcDbSequenceEnd count: $acdbend_count\n"; 

    # Clear tagcheck & orphan, seqend, acdbsequenceend counts before next run of xparser
    %tagcheck = ();
    $attrib_orphan_count = 0;

...
# Program variables go here:
 ...
 our %hof_blocks; # Hash of blocks - this is the desired result of each file parsed
 our @tags = qw(HANDLE BLOCKNAME);      # Array of tag key names in order of discovery (i.e CAD order)
 our %tagcheck; # Hash to keep track unique tags
    ...
    while (<$X_FILE>) {
        ...
        for ($line) {
            ...
            # Keep looking for more attributes by setting state to ATTRIBUTE
                ...
                $line =~ s/\r?\n$//;
                $tagkey = $line;
                print "Key is $line Value is $tagvalue\n";
            addnewtag ($line);
            }

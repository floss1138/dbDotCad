...
## SORCE AND DESTINATION NAMES ## 

# SOURCE CPS, TO DESTINATION CPD, NAMES 
#
# Comma deliminated list or source blocks and corresponding destination blocks
...
# s1 is for site 1, t1 is for template 1, x1 is for cross site 1, h1 hostnames for site 1
fname_match1="^[0-9]+-[0-9]+-[0-9]+-[A-Z]+_.*(\.txt|\.TXT)"
fname_match2="^[stxh]\d+_([0-9]+-){3}[A-Z]+_.*(\.txt|\.TXT)"
...
doc_title="(^[stxh]\d+_[0-9]+-[0-9]+-[0-9]+)"
...
# String to hold truncated blockname used as sheetname - this has a 31 char limit in Excel
    my $string31;
    foreach my $k ( keys %block_id_excel ) {
        ...
        $string31 = $block_id_excel{$k};
        my $len = length($string31);
        if ( $len > 31 ) {
            $block_id_excel{$k} = substr( $string31, 0, 31 );
            print
"\n Length of sheetname has been truncated from $len down to 31 characters:\n $block_id_excel{$k}\n";
        }
    }

# print "Excel safe sheet names now exist but there is still a 31 char limit:\n";
#  print Dumper (\%block_id_excel);
...
'Columns B & C containing the Mongo _id & untranslated block name are intentionally hidden'
    );
...
# If there is a missing sheet name then what came back from the database may have missing data or was not updated in some way
                ...
                $current_sheet->set_column( 'B:C', undef, undef, 1, 0, 0 )
                  ; # This hides column B to C which is the _id and BLOCKNAME.  Blockname shoul be teh same on every line.
...
# need to reverse the block_id_excel hash, to access via worksheet as the value
  ...
  # print Dumper ( \%block_id_excel);
                my @cols =
                  split( ',', $excel_cols{$worksheet_name} );   # split on comma
...
#                Following is for dbug only.  Removing of periods is done when json is written.  $keys[$i] 
#                my @mongo_keys = @keys;
...
# remove leading zero from doctitle in the config match, as this is used in primary key s1_02-03-2475 should become s1_2-3-2475

                print
"Filename is $attfile, doctitle is $doctitle from regex $config{doc_title}\n";
...
# Take pathwithfilename, dbname , ref to primkeys as arguements
                ...
                print
"Mongo upload results will be written to: $config{done_dir}$doctitle.attout_result.txt\n";
                system(
"mongo < $config{done_dir}$doctitle.attout.js > $config{done_dir}$doctitle.attout_result.txt"
                );
                print
"Mongo query results will be written to: $config{done_dir}$doctitle.attin.json\n";
...
Check if data contains source or destination blocks, eg PINAR, PINAL
Schedules typically go from source to destination so if there are no sources or destinations, no need for a schedule tab
If sources exist, create hash of cable numbers where number is key and value is a count 
For each key, if key exists increment count

For each key/cable number, query database for same site and area as the CAD document belongs to

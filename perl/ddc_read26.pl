...
# Best practice is to always put a version number in the block name, even if only the attribute order is changed.
    ...
    #BEGIN TO CREATE EXCEL FILE HERE....??

    # Also reate a spread sheet of the attin file:
    my $row = 3
      ; # Starting row number used to populate spread sheet to allow for a heading if necessary
...
# my $worksheet3 = $workbook->add_worksheet('Readme');            # Worksheet 3 is a readme
    ...
    my $time = time_check();
    $worksheetrm->write( 'B2', "Created by ddc reader $VERSION at $time" )
      ;    #  worksheet created for info, notices & copyright
    $worksheetrm->write( 'B3',
"This software is Free - copyright (c) 2017 by Floss (floss1138\@gmail.com) written for my PDP.  All rights reserved."
    );
    $worksheetrm->write( 'B4',
"You are free to use, copy and distribute this software under the same GPL terms as the Perl 5 programming language."
    );
    $worksheetrm->write( 'B5',
"The Excel module used is copyright (c) John McNamara under GPL http://opensource.org/licenses/gpl-license.php"
    );
...
print "Unique value is: ";
    foreach my $unique_value ( sort values %block_id ) {
        ...
        my $worksheet = $workbook->add_worksheet("$unique_value");
        $worksheet->write( 'B2', "Worksheet for $unique_value, created on $time" );
    }
    print "\n";
...
#As attin is dealing with database content it should be clean unless manually edited
              ...
              my $worksheet_name = $block_id{$block_ident};
        #      print "Value for this tag string is: $block_id{$block_ident}\n";
# Current hash ref to sheet $worksheet_name is:
my $current_sheet = $workbook->get_worksheet_by_name($worksheet_name);

# Write to current sheet here:
$current_sheet->write ( "C3", $block_ident );

$current_sheet->write ( "C$unique_value_count{$worksheet_name}", $block_ident);

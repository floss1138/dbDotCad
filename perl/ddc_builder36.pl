...
# SERVER BUILD SCRIPT FOR dbDotCad RUNNING ON Ubuntu server 12.04
...
print << "GREETINGS";
   ...
   sets up an environment for dbDotCad running on Ubuntu.
   It is about to install onto IP address: $ipaddress
...
system("mkdir -p -m 755 /home/$user/dbdotcad");
system("mkdir -p -m 755 /home/$user/dbdotcad/attout_to_db");
system("mkdir -m 755 /home/$user/dbdotcad/done");
system("mkdir -m 755 /home/$user/dbdotcad/failed");
system("mkdir -m 755 /home/$user/dbdotcad/attout_to_xlsx");
system("mkdir -m 755 /home/$user/dbdotcad/xlsx");
system("mkdir -m 755 /home/$user/dbdotcad/attin");
system("mkdir -m 755 /home/$user/dbdotcad/attvalid");
system("mkdir -m 755 /home/$user/dbdotcad/send_for_review");
system("mkdir -m 755 /home/$user/dbdotcad/TESTFILES");
system("mkdir -m 755 /home/$user/dbdotcad/TEMPLATES");
system("mkdir -p -m 755 /media/data/TESTFILES");
system("mkdir -p -m 755 /media/data/TEMPLATES");

# So that logs from the users directory can be viewed from a web page, 
# a direcotry is added into the document root and an Alias will be used making it look like /home/$user 
...
print "\n Adding Alias in httpd.conf for log directory\n";
system ("echo 'Alias /home/$user/ddclog /var/www/ddclog' >> /etc/apache2/httpd.conf");
...
<a href="/home/$user/ddclog">users log</a>
<p>To access SAMBA share from Windows, copy this link:</p><p>  \\\\$ipaddress\\$user </p>
<p>Use Windows Explorer (Windows key + E)<br> or <br> Click Start, then 'Search programs and files' <br>
Now paste the above link into Explorer or Search. <br>
The 'Enter Network Password' box should eventuallyn open <br>
After validation, the Explorer Window should show the users home directory</p>
<p>Hover over the folder icon in the Explorer window address bar and drag to Desktop or Favourites to create a short cut</p>
</body></html>
...
system("wget https://raw.githubusercontent.com/floss1138/dbDotCad/master/perl/$ddc_read");

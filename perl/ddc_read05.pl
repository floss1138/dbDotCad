...
our $VERSION = '0.0.05';    # Version number of this script
...
# e.g for numeric part of string '123-45-6789-MAD_just a test_file.txt'
# fname_match1="^[0-9]+-[0-9]+-[0-9]+-[A-Z]+_.*"
fname_match1="^[0-9]+-[0-9]+-[0-9]+-[A-Z]+_.*"
fname_match2="UNDEFINED"
...
# It is possible sort { -M ($watch_folder.$b) <=> -M ($watch_folder.$a) } #  sorts by m time but, use brackets around dir.filename to prevent operand type mismatch

        @candidates =
          grep { !/^\./ && -f "$watch_folder/$_" && (/$match/xms) }
          readdir(DIR);

        #  print "\n$match matched the following files >\n";
        #  foreach (@candidates) {
        #   print"$_\n";
        #  }
...
# print the pattern end matches here for debug
# print "Match using following regex:\n";

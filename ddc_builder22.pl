...
# Specify the path to the required mongodb binary here:
# (check https://www.mongodb.org/downloads for the latest)
my $mongodb_latest =
  'http://fastdl.mongodb.org/linux/mongodb-linux-x86_64-ubuntu1204-3.0.1.tgz';
...
print << "GREETINGS";
   ...
   This script $PROGRAM_NAME V$VERSION sets up an environment for CADBox,
   on this machine with IP address: $ipaddress
   created and tested for Ubuntu 12.04 64 bit server.
   mongodb will be installed from \n   $mongodb_latest \n
   The user, $user, will be created.
   You need to be running this script as root.
...
system(
"sudo ln -s /media/data/TESTFILES/ /home/cadbox/media_TESTFILES && sudo ln -s /media/data/TEMPLATES/ /home/cadbox/media_TEMPLATES"
);
...
print << "MESSAGE";
 ...
 To CIFS connect from Windows to $user home folder, click Start & in the search bar:

 \\\\$ipaddress\\$user

 or paste this into Windows Exporer (user name $user & password will be required)
 reset the samba share password wth with: smbpasswd -a $user

 You might want to rm /root/mongod.tar left by the installation
MESSAGE
...
print "\n Creating a startup shell script as /root/start.sh \n";

if ( !open my $STARTUP, '>', '/root/start.sh' ) {
    print "\n  /root/start.sh would not open for writing\n";
}
# write $startup to start.sh file and make it executable
else {
    print $STARTUP "$startup";
    system ("chmod 755 /root/start.sh");
    close $STARTUP or carp "Unable to close /root/start.sh";
}

print "Using crontab -e for the root user, add \n\@reboot /root/startup.sh\n if required \n";
...
Dont try and use 'system' for cd or path chanes or it only change the sub shell running the script and not the root shell
perl has its own chdir command for that...
...
DOCKER:
Docker may be needed, and may be added to this scrip in the future
https://docs.docker.com/installation/ubuntulinux/
Docker is OK with Ubuntu server 14.04 but if using 12.04
...
Added when you install, linux-image-generic-lts-trusty
...
Docker containers will be launched with IP addresses from range 172.17.42.1/24.
